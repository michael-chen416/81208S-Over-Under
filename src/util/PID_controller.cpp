 ...
 */
PID_controller::PID_controller(PIDvalues values){ //write kp ki kd using this
    _values = values;
    _error = _lastError = _sumOfError = _maxSumOfError = _sumErrorRange = 0;
    _outputRange = 1.0;
    _target = 0;
 ...
 * @param values values
 */
void PID_controller::Set_Parameters(PIDvalues values) { //usually odnt need to use this
    _values = values;
}
 ...
 */
void PID_controller::SetTarget(double target) { //mandatory to set target before running calculate, target heading is almost never 0
    _target = target;
}
 ...
 * @param maxSumError 
 */
void PID_controller::SetMaxSumOfError(double maxSumError) { //usualyl we dont use ki, so normally dont need
    _maxSumOfError = abs(maxSumError);
}

double PID_controller::GetError() { //testing
    return _error;
}
